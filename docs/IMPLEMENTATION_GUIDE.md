...
Data pribadi dan file asli tidak kami simpan langsung di blockchain. Blockchain
...
Alur utama sistem kami susun seperti ini:
...
Kami sengaja membuat ringkasan folder ini tidak terlalu detail agar lebih mudah
dibaca:
...
Sebelum menjalankan project, kami menyiapkan:
...
Pertama, kami membuat project di [Supabase](https://supabase.com/). Setelah
project siap, kami buka [dashboard Supabase](https://supabase.com/dashboard)
lalu masuk ke **Project Settings > API**. Dokumentasi resmi Supabase bisa
dibaca di [Supabase Docs](https://supabase.com/docs).

Dari halaman itu kami ambil:

- `Project URL`
- `service_role key`

Kemudian kami masukkan ke file `backend/.env`:
...
Setelah env Supabase disiapkan, kami masuk ke SQL Editor Supabase dan
menyiapkan tabel aplikasi. Jalankan file database sesuai nomor urutan berikut:
...
1. Jalankan schema utama dari `backend/database/01-table.txt` terlebih dahulu.
2. Setelah schema utama berhasil dibuat, jalankan migration tambahan dari
   `backend/database/02-migrations.txt`.
...
terlebih dahulu agar bisa dijalankan sebagai schema awal Supabase. Jangan
menjalankan `02-migrations.txt` sebelum schema utama dari `01-table.txt`
tersedia.
...
Backend SIPEVITA memakai Pinata private network. Pertama, kami buka
...
Setelah credential Pinata siap, nilai yang perlu kami isi di `backend/.env`:
...
dashboard, kami membuka menu **Turnstile**, membuat widget baru, mengisi nama
...
Jika kami ingin memakai chaincode dari lokasi lain, kami bisa override dengan:
...
Flag `--init-ok` dipakai karena script masih punya guard lama terkait
non-determinism `InitLedger`. Source chaincode saat ini sudah memakai timestamp
transaksi dari Fabric melalui `_getTxTimestamp(ctx)`, tetapi tetap ada fallback
`new Date()` jika timestamp tidak tersedia. Untuk jaringan multi-endorser,
kami menghindari transaksi yang bisa masuk ke fallback tersebut.

### 9.7 Buat Wallet Aplikasi

Backend membutuhkan identity Fabric untuk memanggil chaincode. Kami membuat
...
## 12. Alur Penggunaan Lokal

Urutan penggunaan lokal yang kami ikuti:
...
Kami menyiapkan helper curl di folder:
...
Kami tidak menggunakan `docker system prune`, `docker volume prune`, atau
`docker network prune` untuk reset project ini kecuali benar-benar memahami
dampaknya.

## 15. Catatan yang Perlu Diperhatikan

Beberapa hal yang masih perlu diperhatikan saat implementasi lokal:

- `generate-channel-artifacts.sh` belum tersedia, jadi channel artifact masih
  dibuat manual dengan `configtxgen`.
- `generate-connection-profile.sh` belum tersedia, jadi connection profile
  perlu dicek ulang jika crypto material diregenerate.
- `connection-raft.json` bisa stale jika sertifikat TLS digenerate ulang.
- File wallet, private key, MSP, TLS generated, dan credential `.env` tidak
  boleh di-commit.
- Helper `backend/test-curl/*.sh` sudah disesuaikan ke default port `3001`.
